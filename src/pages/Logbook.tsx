...
export function Logbook() {
  ...
  const handleAdd = () => {
    ...
    addLog({
      date: newDate, tankId: newTankId, category: newCat,
  ...
  return (
    <div>
      ...
      <div className="card mb-2">
        ...
        <div className="grid-2">
          <div>
            ...
            <input id="log-date" className="input" type="date" value={newDate} onChange={e => setNewDate(e.target.value)} />
          </div>
          <div>
            <label htmlFor="log-tank">Tank</label>
            <select id="log-tank" className="select" value={newTankId} onChange={e => setNewTankId(e.target.value)}>
      ...
      <div className="card">
        {filtered.length === 0 && (
          <p className="text-sm text-muted" style={{ padding: '0.75rem 0' }}>
            {data.logs.length === 0
...
function LogRow({ entry, tankName, editing, onEdit, onSave, onCancel, onDelete }: {
  ...
  // Ref to the pencil button — focus returns here after edit closes
  ...
  if (editing) {
    return (
      <div
        style={{ padding: '0.6rem 0.75rem', borderBottom: '1px solid var(--color-divider)' }}
        onKeyDown={e => {
      ...
      >
        <div className="text-xs text-muted" style={{ marginBottom: 6 }}>
          {cat?.emoji} {cat?.label} \u00b7 {tankName} \u00b7 {fmtDate(entry.date)}
        </div>
        <div className="flex-row gap-1">
          ...
          />
          <button className="btn btn-primary btn-sm" onClick={handleSave} aria-label="Save changes">
            <Check size={14} aria-hidden="true" />
          </button>
          <button className="btn btn-ghost btn-sm" onClick={handleCancel} aria-label="Cancel editing">
    ...
    <div style={{
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        <div className="flex-row gap-1" style={{ alignItems: 'center' }}>
          ...
          <span className="text-xs text-muted">{tankName}</span>
        </div>
        <div className="text-xs text-muted" style={{ marginTop: 2 }}>
          {fmtDate(entry.date)}
          {summarize(entry) && ` \u00b7 ${summarize(entry)}`}
