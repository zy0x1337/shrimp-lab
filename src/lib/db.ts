import type { AppData, TankProfile, LogEntry, BreedingEntry, BreedingPair, BreedingLine, AppSettings } from './types'
...
export function loadData(): AppData {
  try {
    ...
    return {
      ...
      breedingLines: data.breedingLines ?? [],
...
// Breeding entries (berried females)
...
// Breeding Pairs
export function addBreedingPair(data: AppData, pair: Omit<BreedingPair, 'id' | 'createdAt' | 'litterIds'>): AppData {
  const updated = { ...data, breedingPairs: [...data.breedingPairs, { ...pair, id: uid(), litterIds: [], createdAt: new Date().toISOString() }] }
  saveData(updated); return updated
}
export function updateBreedingPair(data: AppData, id: string, patch: Partial<BreedingPair>): AppData {
  const updated = { ...data, breedingPairs: data.breedingPairs.map(p => p.id === id ? { ...p, ...patch } : p) }
  saveData(updated); return updated
}
export function deleteBreedingPair(data: AppData, id: string): AppData {
  const updated = { ...data, breedingPairs: data.breedingPairs.filter(p => p.id !== id) }
  saveData(updated); return updated
}

// Breeding Lines
export function addBreedingLine(data: AppData, line: Omit<BreedingLine, 'id' | 'createdAt'>): AppData {
  const updated = { ...data, breedingLines: [...data.breedingLines, { ...line, id: uid(), createdAt: new Date().toISOString() }] }
  saveData(updated); return updated
}
export function updateBreedingLine(data: AppData, id: string, patch: Partial<BreedingLine>): AppData {
  const updated = { ...data, breedingLines: data.breedingLines.map(l => l.id === id ? { ...l, ...patch } : l) }
  saveData(updated); return updated
}
export function deleteBreedingLine(data: AppData, id: string): AppData {
  const updated = { ...data, breedingLines: data.breedingLines.filter(l => l.id !== id) }
  saveData(updated); return updated
}
...
export function importData(json: string): AppData | null {
  try {
    ...
    const data: AppData = {
      ...
      breedingLines: parsed.breedingLines ?? [],
